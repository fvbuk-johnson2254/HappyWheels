...
import b2NullContact from "@/Box2D/Dynamics/Contacts/b2NullContact";
...
export default class b2ContactManager extends b2PairCallback {
    ...
    public m_nullContact = new b2NullContact();
    ...
    public override PairAdded(param1, param2) {
        ...
        if (
            this.m_world.m_contactFilter != null &&
            this.m_world.m_contactFilter.ShouldCollide(_loc3_, _loc4_) == false
        ) {
            return this.m_nullContact;
        }
        var _loc7_ = b2Contact.Create(
            _loc3_,
            _loc4_,
    ...
    public override PairRemoved(param1, param2, param3) {
        if (param3 == null) {
            return;
        }
        var _loc4_: b2Contact = param3 as b2Contact;
        if (_loc4_ == this.m_nullContact) {
            return;
        }
        this.Destroy(_loc4_);
    }

    public Destroy(param1: b2Contact) {
        ...
        var _loc4_: number = param1.m_manifoldCount;
        if (_loc4_ > 0 && Boolean(this.m_world.m_contactListener)) {
            _loc7_ = _loc2_.m_body;
            _loc8_ = _loc3_.m_body;
            _loc9_ = param1.GetManifolds();
            _loc10_ = b2ContactManager.s_evalCP;
            _loc10_.shape1 = param1.m_shape1;
            ...
            while (_loc11_ < _loc4_) {
                ...
                while (_loc13_ < _loc12_.pointCount) {
                    ...
                    _loc10_.position = _loc7_.GetWorldPoint(
                        _loc14_.localPoint1,
                    );
                    _loc15_ = _loc7_.GetLinearVelocityFromLocalPoint(
                        _loc14_.localPoint1,
                    );
                    _loc16_ = _loc8_.GetLinearVelocityFromLocalPoint(
                        _loc14_.localPoint2,
                    );
                    _loc10_.velocity.Set(
                        _loc16_.x - _loc15_.x,
                        _loc16_.y - _loc15_.y,
                    );
