...
@boundClass
export default class Building1 extends LevelItem {
    private sprite: Sprite;

    constructor(param1: Special) {
        super();
        var _loc2_: Building1Ref = null;
        var _loc4_: number = NaN;
        var _loc6_: number = NaN;
        var _loc7_: number = NaN;

        _loc2_ = param1 as Building1Ref;
        var _loc3_ = new b2PolygonDef();
        _loc3_.friction = 0.3;
        _loc3_.restitution = 0.1;
        _loc3_.filter.categoryBits = 8;
        _loc4_ = _loc2_.floorWidth * 300;
        var _loc5_: number = _loc2_.numFloors * 165 + 100;
        _loc6_ = 0;
        _loc7_ = 0;
        ...
        this.sprite.graphics.beginBitmapFill(
            _loc9_.getTexture(Building1Ref.B1_ROOF),
            _loc10_,
            true,
            false,
        );
        this.sprite.graphics.drawRect(_loc6_, _loc7_, _loc4_, 100);
        this.sprite.graphics.endFill();
        _loc7_ += 100;
        _loc10_ = new Matrix(1, 0, 0, 1, _loc6_, _loc7_);
        this.sprite.graphics.beginBitmapFill(
            _loc9_.getTexture(Building1Ref.B1_FLOOR),
            _loc10_,
            true,
            false,
        );
