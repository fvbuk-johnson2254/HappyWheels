import { boundClass } from 'autobind-decorator';
import ByteArray from "flash/utils/ByteArray";

@boundClass
export default class Base64 {
    private static BASE64_CHARS: string;
    public static version: string = "1.0.0";

    constructor() {
        throw new Error("Base64 class is static container only");
    }

    public static encode(param1: string): string {
        var _loc2_ = new ByteArray();
        _loc2_.writeUTFBytes(param1);
        return Base64.encodeByteArray(_loc2_);
    }

    public static encodeByteArray(param1: ByteArray): string {
        var _loc3_: any[] = null;
        var _loc5_: number = 0;
        var _loc6_ = 0;
        var _loc7_: number = 0;
        var _loc2_: string = "";
        var _loc4_ = new Array(4);
        param1.position = 0;
        while (param1.bytesAvailable > 0) {
            _loc3_ = new Array();
            _loc5_ = 0;
            while (_loc5_ < 3 && param1.bytesAvailable > 0) {
                _loc3_[_loc5_] = param1.readUnsignedByte();
                _loc5_++;
            }
            _loc4_[0] = (_loc3_[0] & 252) >> 2;
            _loc4_[1] = ((_loc3_[0] & 3) << 4) | (_loc3_[1] >> 4);
            _loc4_[2] = ((_loc3_[1] & 15) << 2) | (_loc3_[2] >> 6);
            _loc4_[3] = _loc3_[2] & 63;
            _loc6_ = _loc3_.length;
            while (_loc6_ < 3) {
                _loc4_[_loc6_ + 1] = 64;
                _loc6_++;
            }
            _loc7_ = 0;
            while (_loc7_ < _loc4_.length) {
                _loc2_ += Base64.BASE64_CHARS.charAt(_loc4_[_loc7_]);
                _loc7_++;
            }
        }
        return _loc2_;
    }

    public static decode(param1: string): string {
        var _loc2_: ByteArray = Base64.decodeToByteArray(param1);
        return _loc2_.readUTFBytes(_loc2_.length);
    }

    public static decodeToByteArray(param1: string): ByteArray {
        var _loc6_: number = 0;
        var _loc7_: number = 0;
        var _loc2_ = new ByteArray();
        var _loc3_ = new Array(4);
        var _loc4_ = new Array(3);
        var _loc5_ = 0;
        while (_loc5_ < param1.length) {
            _loc6_ = 0;
            while (_loc6_ < 4 && _loc5_ + _loc6_ < param1.length) {
                _loc3_[_loc6_] = Base64.BASE64_CHARS.indexOf(
                    param1.charAt(_loc5_ + _loc6_),
                );
                _loc6_++;
            }
            _loc4_[0] = (_loc3_[0] << 2) + ((_loc3_[1] & 48) >> 4);
            _loc4_[1] = ((_loc3_[1] & 15) << 4) + ((_loc3_[2] & 60) >> 2);
            _loc4_[2] = ((_loc3_[2] & 3) << 6) + _loc3_[3];
            _loc7_ = 0;
            while (_loc7_ < _loc4_.length) {
                if (_loc3_[_loc7_ + 1] == 64) {
                    break;
                }
                _loc2_.writeByte(_loc4_[_loc7_]);
                _loc7_++;
            }
            _loc5_ += 4;
        }
        _loc2_.position = 0;
        return _loc2_;
    }
}